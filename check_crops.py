...
IMG_DIR = Path('real_aug2/crops/images')
LBL_DIR = Path('real_aug2/crops/labels')
...
# 8 ornek sec (farkli etiketlerden)
indices = [0, 3, 6, 9, 18, 21, 24, 30]
samples = [imgs[i] for i in indices if i < len(imgs)]
...
for img_path in samples:
    ...
    cv2.putText(ov, img_path.stem[-20:], (8,35),
                cv2.FONT_HERSHEY_SIMPLEX, 0.6, (255,255,255), 2)
    panels.append(cv2.resize(ov, (600, 600)))

grid = np.vstack([np.hstack(panels[:4]), np.hstack(panels[4:])])
cv2.imwrite('check_crops.jpg', grid, [cv2.IMWRITE_JPEG_QUALITY, 92])
print('ok - check_crops.jpg')
