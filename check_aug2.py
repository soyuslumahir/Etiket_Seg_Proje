...
IMG_DIR = Path('real_aug2/images')
LBL_DIR = Path('real_aug2/labels')
...
for img_path in samples:
    ...
    panels.append(cv2.resize(ov, (400, 400)))

# 5 satir x 6 sutun
rows = [np.hstack(panels[i*6:(i+1)*6]) for i in range(5)]
grid = np.vstack(rows)
cv2.imwrite('check_aug2.jpg', grid, [cv2.IMWRITE_JPEG_QUALITY, 88])
print('ok - check_aug2.jpg')
