...
for img_path in samples:
    ...
    # Dosya adi + siniflar
    n_classes = len(set(classes_found))
    info = f'{img_path.stem[-30:]}  ({n_classes} sinif)'
    cv2.putText(ov, info, (10,40), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255,255,255), 2)

    out_panels.append(cv2.resize(ov, (900, 675)))

grid = np.vstack([np.hstack(out_panels[:3]), np.hstack(out_panels[3:])])
cv2.imwrite('preview_real3.jpg', grid, [cv2.IMWRITE_JPEG_QUALITY, 90])
print('ok - preview_real3.jpg kaydedildi')
