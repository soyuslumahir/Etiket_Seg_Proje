...
originals = sorted(IMG_DIR.glob('usb_frame*.jpg'))
...
for img_path in originals:
    ...
    if lbl_path.exists():
        ...
        for line in lines:
            ...
            cv2.polylines(ov, [pts], True, bgr, 6)
            cx_pt = int(np.mean(pts[:,0]))
            cy_pt = int(np.mean(pts[:,1]))
            cv2.putText(ov, CLASS_NAME.get(cid,'?'), (cx_pt, cy_pt),
                        cv2.FONT_HERSHEY_SIMPLEX, 1.5, bgr, 3)

    # 1200x900 olarak kaydet
    panels.append(cv2.resize(ov, (1200, 900)))

grid = np.hstack(panels)
cv2.imwrite('check_original.jpg', grid, [cv2.IMWRITE_JPEG_QUALITY, 92])
print('ok - check_original.jpg')
